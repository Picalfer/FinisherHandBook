...
class MyAdapter(listArray:ArrayList<ListItem>, context: Context): RecyclerView.Adapter<MyAdapter.ViewHolder>() {
    ...
    class ViewHolder(view: View): RecyclerView.ViewHolder(view) {
        ...
        fun bind(listItem: ListItem, context: Context) {
            ...
            var textCon = listItem.contentView.substring(0,3) + "..."
            ...
            itemView.setOnClickListener() {
                ...
                val i = Intent(context, ContentActivity::class.java).apply {
                    ...
                    putExtra("content", listItem.contentView)
    ...
    fun updateAdapter(listArray: List<ListItem>) {
        listArrayR.clear() //чистим старый массив
        listArrayR.addAll(listArray) // заполняем новой информацией
        notifyDataSetChanged() // уведомляем адаптор, что ему нужно оновиться
    }
}